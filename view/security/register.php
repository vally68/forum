    ...
    <form action="?ctrl=security&action=register" method="post">
        ...
        <input type="password" name="password_repeat" id="password_repeat" minlength="12"
            pattern="(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[\W_]).{12,}"
            required>
    <legend>Pour accéder au forum, vous devez accepter les C.G.U.</legend>

    <div>
        <input type="checkbox" id="CGU" name="CGU" required />
        <label for="CGU">J'accepte les 
            <a href="index.php?ctrl=security&action=cgu" target="_blank">Conditions Générales d'Utilisation</a>
        </label>
    </div>
